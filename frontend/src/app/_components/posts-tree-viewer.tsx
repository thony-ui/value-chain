...
import { useGetPostDescendants } from "@/queries/use-get-post";
...
function calculatePayouts(
  ...
  // Find the path from startNodeId up to the root
  const path: PostTreeNode[] = [];
  function findPath(node: PostTreeNode): boolean {
    if (node.post.id === startNodeId) {
      path.push(node);
      return true;
    }
  ...
  findPath(tree);
  path.reverse(); // root -> startNode

  const payouts: Record<string, number> = {};
  if (path.length === 0) return payouts;

  const ownerId = path[0].post.id;
...
export function PostsTreeViewer({ postId }: { postId: string }) {
  const { data: tree, isLoading, error } = useGetPostDescendants(postId);
  ...
  useEffect(() => {
    if (tree && transactionNodeId) {
      setPayouts(calculatePayouts(tree, transactionAmount, transactionNodeId));
      // Find the path from transactionNodeId up to the root for coloring
      const path: PostTreeNode[] = [];
      function findPath(node: PostTreeNode): boolean {
        if (node.post.id === transactionNodeId) {
          path.push(node);
          return true;
        }
        for (const child of node.children) {
      ...
      findPath(tree);
      path.reverse(); // root -> transactionNode
      setPayoutPath(path.map((n) => n.post.id));
    } else {
      setPayouts({});
      setPayoutPath([]);
    }
  }, [tree, transactionNodeId, transactionAmount]);
  ...
  const elements = tree
    ? (() => {
        ...
        function traverse(node: PostTreeNode, parentId?: number) {
          const id = nodeId++;
          nodes.push({
            data: {
              id: node.post.id,
              label: node.post.id.slice(0, 8),
              user_id: node.post.user_id,
              created_at: node.post.created_at,
              isInPayoutPath: payoutPath.includes(node.post.id),
            },
          });
          if (parentId !== undefined) {
            edges.push({
              data: {
                source: nodes[parentId].data.id,
                target: node.post.id,
                isInPayoutPath:
                  payoutPath.includes(node.post.id) &&
                  payoutPath.includes(nodes[parentId].data.id),
              },
            });
          }
  ...
  const stylesheet = [
    ...
    },
    {
      ...
      style: {
        width: 2,
        "line-color": "#9D00FF",
        "target-arrow-color": "#6b7280", // Changed to darker gray for better contrast
      ...
      <div className="flex flex-col items-center pb-6 pt-2">
                ...
                <SelectContent className="border border-gray-200 rounded-lg">
                  {tree &&
                    (() => {
                      ...
                      const flat: PostTreeNode[] = [];
                      function flatten(node: PostTreeNode) {
                        flat.push(node);
        ...
        {elements.length > 0 ? (
          ...
          <div className="text-center text-gray-500 mt-8 bg-gray-50 rounded-lg p-6 border border-gray-200">
            ...
            <p className="text-sm font-medium mt-1">No tree found.</p>
          </div>
        )}
      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                        ...
                        <p className="text-gray-600 text-sm">
                          {new Date(tree.post.created_at).toLocaleString()}
                        </p>
                      </div>
                    </div>
                  );
